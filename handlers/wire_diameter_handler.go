...
// @Success 200 {array} models.WireDiameterValues
// @Failure 500 {string} string "Внутренняя ошибка сервера"
// @Router /wire_diameter [get]
func GetWireDiameter(c *fiber.Ctx) error {
	...
	area_value := c.Query("area_value")

	if material_name == "" || welding_type_name == "" || katet_value == "" || seam_type_name == "" || area_value == "" {
		return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{
			"error": "Все параметры запроса обязательны",
		})
	}

	var wireDiameter []models.WireDiameterValues
	query := `select distinct wire_diameter.diameter_value from tnsh
			  ...
			  and seam_types.seam_type_name = $4 and cross_sec_areas.area_value = $5 and materials.welding_check = 'true'`
	err := database.DB.Select(&wireDiameter, query, material_name, welding_type_name, katet_value, seam_type_name, area_value)
	if err != nil {
		...
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
			"error": "Не удалось получить значение wire_diameter",
		})
	}

	return c.JSON(wireDiameter)
}
